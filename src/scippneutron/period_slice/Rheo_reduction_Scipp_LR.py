...
def rheo_waveform_params(rheo_binned, method):
    ...
    if method == 'curve_fit':
        ...
        rheo_for_fit.coords['time'] = rheo_for_fit.coords['time'] - rheo_for_fit.coords['time'][0]
        ...
        A_0 = ((sc.max(rheo_for_fit) - sc.min(rheo_for_fit)) / sc.scalar(2, unit='')).data
        Aoff_0 = sc.mean(rheo_for_fit).data
        mask_first_p = rheo_for_fit.coords['time'] < T_0
        Toff_0 = rheo_for_fit.coords['time'][mask_first_p][(rheo_for_fit[mask_first_p] == sc.max(rheo_for_fit[mask_first_p])).data][0] - T_0 / 4

        print({'A': A_0, 'T': T_0, 'Toff': Toff_0, 'Aoff': Aoff_0})
        par, _ = sc.curve_fit(['time'], sine_wave, rheo_for_fit, p0 = {'A': A_0, 'T': T_0, 'Toff': Toff_0, 'Aoff': Aoff_0})
...
def period_edges(signal, plot_flag, value=None):
    ...
    if plot_flag:
        plt.figure()
        plt.plot(np.diff(period_edges), label='', linestyle='', marker='o', markerfacecolor='none')
        plt.xlabel('Samples')
...
def ppT_autocorrelation(signal, plot_flag=None):
    ...
    if plot_flag:
        ...
        plt.plot(locs, R[locs], marker='o', linestyle='None', markeredgecolor='r', markerfacecolor='none', label='Peaks')        
        plt.xlabel('Samples')
    ...
    if plot in ['plot', 'overplot']:     
        if plot == 'plot':
            ...
            if periods is not None:
                ...
                plt.plot(time_rheo[idx_start:idx_end].values - time_rheo[idx_start].values, rheo[idx_start:idx_end].values, marker='.', linestyle='', color='k', markersize=3)
                plt.plot(time_fft[idx_start:idx_end].values - time_fft[idx_start].values, rheo_fitted[idx_start:idx_end].values, marker='', linestyle='-', color='r', markersize=1)
            else:
        ...
        if plot == 'overplot':
            ...
            if periods is not None:
                for i in range(periods[0] - 1, periods[1]):
                    ...
                    plt.plot(time_rheo[idx_start:idx_end].values - time_rheo[idx_start].values, rheo[idx_start:idx_end].values, marker='.', linestyle='', color='k', markersize=1)
                    plt.plot(time_fft[idx_start:idx_end].values - time_fft[idx_start].values, rheo_fitted[idx_start:idx_end].values, marker='', linestyle='-', color='r', markersize=1)
            else:
                for i in range(len(rheo) // N_period):
                    ...
                    idx_end = (i + 1) * N_period
                    plt.plot(time_rheo[idx_start:idx_end].values - time_rheo[idx_start].values, rheo[idx_start:idx_end].values, marker='.', linestyle='', color='k', markersize=1)
                    plt.plot(time_rheo[idx_start:idx_end].values - time_rheo[idx_start].values, rheo_fitted[idx_start:idx_end].values, marker='', linestyle='-', color='r', markersize=1)
            plt.xlabel('Time [ns]')
            plt.ylabel('Rheometer Deflection Angle')
            # plt.legend()
            plt.show()
...
def time_at_sample(event_time_zero, event_time_offset, distance_to_moderator, distance_to_sample):
    return (
        # Think on this
        event_time_offset.to(unit=event_time_zero.unit) * distance_to_sample / distance_to_moderator + event_time_zero
...
def period_slice(time_at_sample, T, Toff, N_slices):
    ...
    # print(first_edge)
    return (
        ((N_slices * ((time_at_sample - first_edge) / (T.to(unit=time_at_sample.unit)))) % N_slices).to(dtype='int32')
...
def period_slice_trigger(time_at_sample, period_times, N_slices):
    ...
    func = sc.DataArray(period_times[1:] - period_times[:-1], coords={'time_at_sample': period_times})
    ...
    return (
        ((N_slices * ((time_at_sample - period_start) / period_duration)) % N_slices).to(dtype='int32')
    )


def dead_time_correction(events, bad_events, proton_charge, tdead, TOFmin, TOFmax, TOFbin):
    import matplotlib.pyplot as plt 


    # Grid on TOF placed at the edges of the bins
    tof_bin_edges = sc.arange('event_time_offset', TOFmin - TOFbin / 2, TOFmax + TOFbin / 2, TOFbin, unit='us', dtype='float64')
