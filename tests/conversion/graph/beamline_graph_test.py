...
def test_two_theta_has_correct_keys():
    assert set(beamline.two_theta().keys()) == {'scattered_beam', 'incident_beam', 'two_theta'}
...
def test_Ltotal_has_correct_keys():
    ...
    assert set(beamline.Ltotal(scatter=True).keys()) == {'scattered_beam', 'incident_beam', 'L1', 'L2', 'Ltotal'}


def test_beamline_has_correct_keys():
    assert set(beamline.beamline(scatter=False).keys()) == {'Ltotal'}
    assert set(beamline.beamline(scatter=True).keys()) == {'scattered_beam', 'incident_beam', 'L1', 'L2', 'Ltotal',
                                                           'two_theta'}
