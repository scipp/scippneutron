...
def make_array(
    *, nevent: int = 1000, wav_min, wav_max, source_position: sc.Variable
) -> sc.DataArray:
    wavelength = sc.array(
        dims=['event'],
    ...
    events = sc.DataArray(
        sc.ones(sizes=wavelength.sizes),
        coords={'wavelength': wavelength, 'pixel': sc.arange('event', nevent) % 2},
    )
    da = events.group('pixel')
    da.coords['source_position'] = source_position
    da.coords['sample_position'] = sc.vector([1.0, 2.0, 3.0], unit='m')
    da.coords['position'] = sc.vectors(
        dims=['pixel'], values=[[1.0, 2.0, 5.0], [1.1, 2.2, 5.5]], unit='m'
    )
    return da
...
def test_single_chopper_stitched_data_yields_correct_wavelength_histogram():
    ...
    stitched_wavelength = stitched.transform_coords(
        'wavelength', graph=graph.tof.kinematic("tof")
    )
