...
def data_array():
    ...
    return sc.DataArray(
        ...
        coords={'position':
                ...
                'source_position': sc.vector(value=np.array([0, 0, -10]),
                                             unit='m'),
...
def test_position(data_array):
    assert sc.identical(scn.position(data_array),
                        data_array.coords['position'])
...
def test_L1(data_array):
    assert sc.identical(scn.L1(data_array),
                        sc.scalar(np.sqrt(1 + 100), unit='m'))


def test_L2(data_array):
    assert sc.identical(scn.L2(data_array),
                        sc.array(dims=['position'],
                                 values=np.sqrt([1 + 1, 0, 1 + 1]),
                                 unit='m'))
...
def test_Ltotal_no_scatter(data_array):
    assert sc.identical(scn.Ltotal(data_array, scatter=False),
                        sc.array(dims=['position'],
                                 values=np.sqrt([1 + 100, 1 + 100, 121]),
                                 unit='m'))
